...
if Fp > F_table:
    ...
    while True:
        ...
        x12 = sum(x_exper_neight[0])
        x23 = sum(x_exper_neight[1])
        x13 = sum(x_exper_neight[2])
        x123 = sum(x_exper_neight[3])
        ...
        try:
            ...
            print("y = {} + {}*x1 + {}*x2 + {}*x3 + {}*x1*x2 + {}*x1*x3 + {}*x2*x3 + {}*x1*x2*x3".format(par(b0), par(b1), par(b2), par(b3), par(b12), par(b13), par(b23), par(b123)))
            
            y_control = []
            for i in range(len(x_factor_experiment[0])):
                y_control.append(b0 + b1*x_factor_experiment[0][i] + b2*x_factor_experiment[1][i] + b3*x_factor_experiment[2][i] + b12 * x_exper_neight[0][i] + b13 * x_exper_neight[1][i] + b23 * x_exper_neight[2][i] + b123 * x_exper_neight[2][i])
